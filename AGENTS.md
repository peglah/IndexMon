...
- **Apprise alerts**: Calls Python `apprise` (pip, installed in Dockerfile) via `child_process.execFile`. `APPRISE_URLS` comma-separated. Skipped if `APPRISE_URLS` unset. In-memory dedup via `alertedDownIds` Set (resets on restart). `ALERT_DELAY_M` (default `0`) delays alert until indexer has been continuously down for that duration. For `ntfy://` URLs, `avatar_url` pointing to the favicon on GitHub raw is injected automatically for notification icons. `--attach /usr/share/nginx/html/favicon.svg` sends the SVG as file attachment.
