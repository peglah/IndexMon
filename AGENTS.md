...
- **Apprise alerts**: POST to `{APPRISE_API_URL}/notify`. `APPRISE_URLS` comma-separated. Skipped if `APPRISE_API_URL` unset. In-memory dedup via `alertedDownIds` Set (resets on restart). `ALERT_DELAY_M` (default `0`) delays alert until indexer has been continuously down for that duration.
