...
| `ADMIN_PASSWORD_HASH` | No | random on each startup | Bcrypt hash. Generate via `scripts/hash.sh yourpass` or `docker run --rm ghcr.io/peglah/indexmon hash-password yourpass`. If unset, a random password is generated and printed to logs. |
